...
export default function DashboardUser3() {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      const filtered = allDocs.filter(doc => {
        ...
        return status === '' || status === 'DRAFT' || status.includes('USER3');
      });
  ...
  const getProgressHistory = (status) => {
    ...
    if (s.includes('USER3')) return 'Sudah di User 3';
    ...
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#f3f4f6', fontFamily: 'sans-serif', margin: 0 }}>
      ...
      <div style={{ flex: 1, padding: '30px', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', borderBottom: '1px solid #e5e7eb', paddingBottom: '15px' }}>
          <h1 style={{ margin: 0, fontSize: '18px', color: '#1f2937' }}>Halo, Pengguna (ID)</h1>
              ...
              <tbody>
                {loading ? (
                  ...
                  data.map((doc) => {
                    ...
                    const creatorName = doc.user?.username || doc.creator || 'User 3 (Anda)';
